...
CTranspFuncsMics::CTranspFuncsMics()
{
...
int CTranspFuncsMics::Send(string sComm, bool bReadAnsw, bool bReadByByte)
{
...
int CTranspFuncsMics::SetRegLVDS(int nAddress, int nReg, int nWrData, bool bRead)
{
    ...
    if( CheckErr(Send("P_INT_COMMAND = " + QString::number((unsigned long)packet.Frame).toStdString())) )
        return -1;
