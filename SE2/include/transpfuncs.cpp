    ...
    if(ptrTr)
    {
        ...
        if (comm.contains("REGUL_CURVE_2")) {
            ...
            * stepPtr = step;
            ...
            head.CommandID = 8; ///< �?дентификатор команды
...
//                    delete [] buf;
            ...
            if (ReadAnsw && comm.contains("RUN")) {
                ...
                QTime time;
                time.start();
                errorCode = ptrTr->SendData(comm.toAscii().data(),&cnt);
...
int TranspFuncs::SetBaudRate(int baud)
{
    if(ptrTr)  {
	if(CheckErr(ptrTr->SetAttribute(VI_ATTR_ASRL_BAUD,baud))) {
	    return -1;
	}
    } else {
	SetError(tr("Do not create a transport class"));
